...
// shifts meaningfully — this is a manual figure, not a live lookup.
const PAYPAL_BUSINESS_EMAIL = 'Yawsei@aol.com';
const USD_TO_GHS_RATE = 16;
...
export default function BookingForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const guestMessage = encodeURIComponent(
      ...
      `Our team will confirm your reservation and reach out to you shortly.\n\n` +
      `📍 *Location:* Tema Community 6, Ghana\n` +
      `📞 *Contact:* +233 542 758 210\n\n` +
      `We look forward to hosting you! 🌟`
    );

    // Open WhatsApp for first admin number
    const waUrl1 = `https://wa.me/233248537939?text=${adminMessage}`;
    // Open WhatsApp for second admin number (slight delay so browser doesn't block)
    const waUrl2 = `https://wa.me/233248537939?text=${adminMessage}`;
  ...
  if (status === 'success') {
    return (
      <div className="bg-card border border-border rounded-2xl p-10 text-center">
        ...
        <h2 className="font-display text-2xl font-medium text-foreground mb-3">Booking Request Sent!</h2>
        {orderNumber && (
        ...
        <p className="text-xs text-muted-foreground mt-3">
          PayPal payment is charged in USD (approx. equivalent at current rate) — final amount can be confirmed via WhatsApp before you pay.
        </p>
      </div>
    );
  }
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-start gap-3 p-3 bg-[#25D366]/10 border border-[#25D366]/20 rounded-xl">
            ...
            <p className="text-xs text-foreground/80">
              A WhatsApp confirmation with your full booking details will be sent to <strong>{form.phone}</strong> upon submission.
            </p>
          </div>
